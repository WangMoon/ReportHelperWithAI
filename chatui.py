...
# </div>
# """
usericon = Image.open('config/favicon.png')
aiicon = Image.open('config/robot.png')
    ...
    if "client" not in state:
        ...
        for message in state.message_history:
            if message["role"] != "system":
                avatar = np.array(aiicon) if message["role"] == "assistant" else np.array(usericon)
        ...
        if user_input:
            ...
            if '名称' in user_input or '项目' in user_input:
                ...
                # 显示消息历史
                st.chat_message("user", avatar=np.array(usericon)).write(user_input)
                st.chat_message("assistant", avatar=np.array(aiicon)).write("正在生成报告，请稍等...")
                ...
                if result.endswith(".docx"):
                    ...
                    st.sidebar.markdown(f"<span style='color:black; font-size:14px;'>🔗 生成文件</span>", 
                                        unsafe_allow_html=True)
                    st.sidebar.markdown(f"""
                                        <div style='
                                            ...
                                            align-items: center;
                                            '> {result}
                                            <a href="demo930/report/templates/index.html" style='
                                                font-size: 16px;
                                        ...
                                        unsafe_allow_html=True)
                    # 在侧边栏中显示文件名
                    # st.sidebar.text_input("🔗 生成文件", value=result, disabled=True)

                    # 在侧边栏添加下载链接
                    # st.sidebar.link_button("Download files", download_url)


                    st.chat_message("assistant",  avatar=np.array(aiicon)).write("报告生成成功，请下载查看。")
                    ...
                    st.chat_message("assistant",avatar=np.array(aiicon)).write("报告生成失败，请重试。")
            else:
                # 用户输入不包含名称或项目，重申系统提示
                st.chat_message("user",avatar=np.array(usericon)).write(user_input)
                state.message_history.append({"role": "user", "content": user_input})
                st.chat_message("assistant",avatar=np.array(aiicon)).write("很抱歉，输入信息不符合要求。请提供项目基本信息，包括项目类型、编号、名称、主持部门和负责人等，我将为您生成一个可供修改的文档，便于您查看和整理。")
