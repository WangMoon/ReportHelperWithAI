        ...
        if user_input:
            ...
            if '名称' in user_input or '项目' in user_input:
                ...
                with st.spinner("请耐心等待..."):
                    ...
                    if "result_history" not in state:
                        state.result_history = []
                    result = generate_docx_file(project_info)
                    state.result_history.append(result)
                    time.sleep(5)

                if all(file.endswith(".docx") for file in state.result_history):
                    state.message_history.append({"role": "assistant", "content": "报告生成成功，请下载查看。"})
                    st.success("文件已生成，请下载查看。")
                    for file in state.result_history:
                        url = f'http://127.0.0.1:5000/{file}'
                        st.link_button(f"  🔗   预   览   文   件  ", url)
                ...
                #     st.link_button("  🔗   预   览   文   件  ", url)
                    ...
                    download_file_sidebar(state.result_history)
...
# def download_file_sidebar(result_files):
#     st.sidebar.markdown(f"<span style='color:black; font-size:14px;'>🔗 生成文件</span>", 
#                                         unsafe_allow_html=True)
#     for file in result_files:
#         # with open(file, "rb") as f:
#         #     st.sidebar.download_button(label=f"  {file}", data=f, file_name=file[10:])
