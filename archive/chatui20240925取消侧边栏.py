...
def chat_ui():
    ...
    if "client" not in state:
        ...
        if "messages" not in state:
            st.session_state.messages = [{"role": "assistant", "content": "欢迎使用科标业项目任务书自动化生成与润色助手。请提供项目基本信息，包括项目类型、编号、名称、主持部门和负责人等，我将为您生成一个可供修改的文档，便于您查看和整理。"}]
        ...
        if user_input:
            ...
            if '名称' in user_input or '项目' in user_input:
                ...
                st.chat_message("assistant").write("正在生成报告，请稍等...")
                ...
                if result.endswith(".docx"):
                    ...
                    st.chat_message("assistant").write("报告生成成功，请下载查看。")
                    ...
                    st.chat_message("assistant").write("报告生成失败，请重试。")
            else:
                # 用户输入不包含名称或项目，重申系统提示
                st.chat_message("user").write(user_input)
                state.message_history.append({"role": "user", "content": user_input})
                st.chat_message("assistant").write("很抱歉，输入信息不符合要求。请提供项目基本信息，包括项目类型、编号、名称、主持部门和负责人等，我将为您生成一个可供修改的文档，便于您查看和整理。")
